<template>
    ...
    <template v-slot:body="props">
      <q-tr :props="props">
        ...
        <q-td key="roles" :props="props">
          <q-tr :props="props" dense autofocus>{{props.row.roles?.map(r=>r.name)}}</q-tr>
        </q-td>
        <q-td key="actions" :props="props" auto-width>
          <q-btn color="blue" label="Изменить Роли" @click="showEditUserDialog(props.row)" size=sm no-caps></q-btn>
        </q-td>
      </q-tr>
    </template>
  ...
  <q-dialog v-model="user_dialog">
    <q-card class="bg-secondary my-card">
      <q-form
        @submit="updateUserRoles">
        <q-card-section>
          <div class="text-h6">Изменить Роли</div>
        </q-card-section>
...
<script lang="ts">
  ...
  const _column_names = ['id', 'username', 'firstName', 'lastName', 'middleName','roles','actions'];
  export default defineComponent({
    name: 'UpdateUserRolesTable',
    setup() {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access,@typescript-eslint/no-unsafe-return
      const roleOptions = computed(() => store.getters['user/getRoleOptions']);
      ...
      function showEditUserDialog(user: IUser) {
        ...
        edited_user.roles = user.roles || [];
      }

      function updateUserRoles(): void {
        void store.dispatch('user/updateUserRoles', edited_user).then(success => {
            if (success) {
              notifyApi.showPositive('Роли успешно измененны');
      ...
      return { rows, columns, user_dialog, edited_user, showEditUserDialog, updateUserRoles, ruleApi,roleOptions,filter };
    }
  });
</script>
